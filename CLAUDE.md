...
6. **Decide `ingestion_events.raw_payload` retention** (`docs/data-recovery.md` has a recommendation - 90-day full-payload retention, prune to just the summary after - but no decision or implementation yet).
...
11. **Add observability and operational safeguards**: monitoring/error reporting, a general audit log (`ingestion_events` now covers imports specifically - status/component/reviewed-exception changes still aren't recorded anywhere), failed-save retry visibility (§8), and a verified restore drill against the now-real backups.
