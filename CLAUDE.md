...
**New here?** [`docs/handoff.md`](docs/handoff.md) is the practical,
numbered-steps path through this file - local setup, shipping a change, and
where everything else lives. This file is the complete reference; that one
is where to start. `AGENTS.md` (repo root) exists only so Codex finds this
file by the name it actually looks for - see that file for why it isn't a
second copy.
...
- `devops/` - Docker Compose files (`docker-compose.yml` for Postgres, `docker-compose.app.yml` for the app service), the app's `Dockerfile`, the NAS deploy script (`deploy.sh`), the day-to-day operations wrapper (`devops.sh` - see §6/§7), the isolated migration tool baked into the app image (`migrate/`, see §7), and backup/maintenance scripts. See §4/§7, and `docs/postgres.md` for the operational detail this section doesn't repeat. Also `restore-ingestion-event.mjs` (`pnpm restore:import`) - restores the normalized tables to a prior `crmDataset` import's exact state; see `docs/data-recovery.md`.
...
- `lib/domain/dataset.ts` - The canonical `Dataset`/`DatasetSubscriber`/`DatasetRecipient`/`DatasetOrder`/`DatasetSubscription`/`DatasetMailing`/`DatasetException`/`DatasetSummary` interfaces - the one shape both `buildSeedFromSpreadsheet` (client) and `buildDatasetFromTables` (server) produce. Also `DatasetImportSkipGroup`/`DatasetImportSummary` - a separate, related shape (what an import skipped and why) that `lib/write-to-tables.ts`'s `writeImport()` returns and `lib/client/shared-state-client.ts` reads off the POST response; lives here for the same cross-layer reason as `Dataset` itself, not because it's part of the dataset shape.
...
**[`docs/handoff.md`](docs/handoff.md) is the practical, numbered-steps version of everything below**, already verified end to end from a fresh clone - this section is the fuller reference underneath it. `pnpm docker:up`/`docker:up:full`/`docker:down` below, and every other local Docker/database operation (a shell, clearing local data, applying a migration, checking health), all go through one script, `devops/devops.sh` - run it with no arguments for the full subcommand list, and see `docs/postgres.md` for the database-facing ones in detail.
...
5. Start local Postgres and apply migrations:
   ...
   `docker:up`/`docker:up:full`/`docker:down` all call `devops/devops.sh` (`up`/`up-full`/`down`), the single definition of the Compose invocation - resolves `.env.local` and the compose files from the repository root regardless of the caller's own working directory, and fails with an actionable message if `.env.local` is missing rather than surfacing Compose's own interpolation error. `docker:up` specifically starts only the `postgres` service (`devops/docker-compose.yml`), working with **zero** auth vars set, deliberately — see §5.
...
Only run `pnpm db:generate` after intentionally changing something under `db/schema/`, then inspect the generated migration in `drizzle/`, commit it, and run `pnpm db:migrate` to apply it to your local Postgres. Both `drizzle-kit` commands need `DATABASE_URL` exported into the shell first (`set -a; source .env.local; set +a` — see step 5 above); neither loads `.env.local` on its own. This is the tool for iterating on a migration you just generated; `devops/devops.sh migrate` (§7) is the separate tool for applying already-committed migrations the same way a real deploy does, and needs no `drizzle-kit`/Node toolchain at all - see `docs/postgres.md` for why they're two different things rather than the same command wrapped twice.
...
2. GitHub Actions' `build-and-push` job computes a build identity (a Mountain Time timestamp formatted at build time, plus the short commit SHA) and builds the app image from `devops/app.Dockerfile` with those passed as build args, pushing it to GHCR as `ghcr.io/marcy-ever/everletter-ops-crm:latest` and `:<commit-sha>`.
...
**Applying a migration by hand** (rare — deploy.sh does this automatically now, see above): `devops/devops.sh migrate` runs the same `devops/migrate/migrate.mjs` against whichever environment it's invoked in, dev box or NAS alike, with no Node/pnpm toolchain required on the host either way — see `docs/postgres.md` for the mechanism and the host-vs-container port distinction it depends on.

**Operational lesson, worth keeping even though the gap it describes is closed:** the NAS ran for weeks against a database that had never been migrated — only the old, pre-normalization `crm_state` table existed, and `drizzle`'s own migration bookkeeping (`drizzle.__drizzle_migrations`) was empty. Nothing caught it, because `GET /api/health` only ever proved Postgres was *reachable* (`SELECT 1`), not that this app's schema actually existed there — the container reported healthy, and `deploy.sh` reported success, the entire time. Recovering also surfaced a second problem: the specific failure that caused the outage (`deploy.sh`'s self-re-exec step failing on the NAS's noexec-mounted `/tmp`) died before the script's own log-file setup ran, so `devops/deploy.txt` showed nothing at all for the whole outage — indistinguishable from "no deploy was attempted." Both are closed now: `/api/health` queries a real table (`select id from mailings limit 1` — fails outright if the schema doesn't exist, not just if Postgres is unreachable), `deploy.sh`'s logging now covers the whole script including its own re-exec, and migrations run automatically as part of every deploy (above). Recorded here because the failure mode — a healthcheck that only proves connectivity, and a log that can't record its own earliest failures — is a real category of gap worth checking for elsewhere, not just a bug that got fixed once.
...
- **This handoff pass**: `docs/handoff.md` (the numbered-steps onboarding path for Marcy and her Codex session) and `AGENTS.md` (a pointer, not a copy - see that file for why) added; this file brought back in line with the tree, including the NAS-episode fixes below.
- **Three operational gaps a real NAS incident exposed, closed.** The NAS ran for weeks against an unmigrated database - see §7's operational-lesson note for the full account. `GET /api/health` now queries a real table instead of a bare connectivity check; `devops/deploy.sh`'s logging now covers its own earliest failures (previously silent); and `devops/deploy.sh` now runs migrations automatically, from an isolated `devops/migrate/` tool baked into the app image (the NAS has no Node toolchain, so this can't run any other way). `devops/devops.sh migrate` uses the identical mechanism, so it's the same command and behavior on a dev box and the NAS.
- **`devops/devops.sh`, a single operations entry point**, and `docs/postgres.md`. Replaced the Compose invocation duplicated across three `package.json` scripts and `devops/deploy.sh` with one script (`up`/`up-full`/`down`/`ps`/`logs`/`restart`/`db-shell`/`db-clear`/`migrate`/`health`); `devops/clear_db.sh` folded in and deleted. `docs/postgres.md` is the practical reference for querying the database directly, including the host-vs-container port distinction every one of these subcommands has to get right.
