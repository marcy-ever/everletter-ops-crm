...
export async function GET() {
  ...
  const batchDates = [...new Set(reviews.map((review) => review.batchDate))];
  const options = batchDates.length ? await db.select({ mailingId: mailings.id, recipientName: mailings.recipientName, shipDate: mailings.scheduledDate, character: subscriptions.character, letterNumber: mailings.letterNumber })
    .from(mailings).innerJoin(subscriptions, eq(mailings.subscriptionId, subscriptions.id)).where(inArray(mailings.scheduledDate, batchDates)).limit(2000) : [];
...
export async function POST(request: Request) {
  try {
    ...
    const [mailing] = await db.select().from(mailings).where(eq(mailings.id, String(mailingId))).limit(1);
    ...
    await db.transaction(async (tx) => {
      ...
      const isFuturePreparation = review.batchDate > everletterTodayIso(new Date());
      if (!isFuturePreparation && mailing.status !== "Mailed") {
      ...
      await tx.insert(auditEvents).values({ actorEmail: review.uploadedBy, kind: "mailingProof", itemKey: key, previousValue: null, newValue: `${isFuturePreparation ? "Prepared" : "Reviewed"} batch photo ${proof.id}` });
