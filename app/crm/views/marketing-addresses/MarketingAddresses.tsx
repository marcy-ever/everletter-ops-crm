...
export default function MarketingAddresses({ state, query, onCustomerClick }: { state: MarketingAddressState | null; query: string; onCustomerClick: (subscriberId: string) => void }) {
  ...
  const rows = state.rows.filter((row) => includesText([row.buyerName, row.email, row.billingAddress, row.recipientName, row.mailingAddress, row.character, row.plan, row.endDate, row.orderNumber], query));
  return <section className="data-panel" aria-label="Marketing addresses">
    ...
    {rows.length ? <div className="marketing-address-list">{rows.map((row) => <article className="marketing-address-card" key={`${row.orderNumber}:${row.subscriberId}`}>
      ...
      <div><span className="section-label">Gift recipient — letter mailing</span><strong>{row.recipientName}</strong><span>{row.mailingAddress}</span><small>{row.character} · {row.plan}</small>{row.endDate ? <span className={row.endingSoon ? "marketing-ending-soon" : ""}>{row.endingSoon ? "Ending soon · " : "Ends "}{formatDate(row.endDate)}</span> : <span>Ongoing subscription</span>}</div>
