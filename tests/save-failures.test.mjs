...
// fact rather than folded into the save-failure count.
...
test("recordSaveFailure increments a running count and records the message", () => {
  const store = createSaveFailureStore();
  store.recordSaveFailure("mailingStatus", "MAIL-1::2", "Network error");
  let snapshot = store.getSnapshot();
  assert.equal(snapshot.failedSaveCount, 1);
  assert.equal(snapshot.lastFailureMessage, "Network error");

  store.recordSaveFailure("componentStatus", "MAIL-1::2::envelope", "HTTP 400");
...
test("a bulk action failing many times counts every failure, not just one", () => {
  ...
  for (let i = 0; i < 120; i += 1) {
    store.recordSaveFailure("mailingStatus", `MAIL-${i}::1`, "Network error");
  }
  assert.equal(store.getSnapshot().failedSaveCount, 120);
});

test("recordSaveSuccess does not clear a prior failure count or message - a later success doesn't undo an earlier failure", () => {
  const store = createSaveFailureStore();
  store.recordSaveFailure("mailingStatus", "MAIL-1::2", "Network error");
  store.recordSaveFailure("mailingStatus", "MAIL-2::3", "HTTP 409: catastrophic deletion refused");
...
test("recordLoadFailure sets loadFailed and its own message, independently of save failures", () => {
  const store = createSaveFailureStore();
  store.recordSaveFailure("mailingStatus", "MAIL-1::2", "save failed");
...
test("subscribe notifies listeners whenever the exposed snapshot actually changes, and the returned unsubscribe stops further notifications", () => {
  ...
  store.recordSaveFailure("mailingStatus", "MAIL-1::2", "x");
  ...
  unsubscribe();
  store.recordSaveFailure("mailingStatus", "MAIL-1::2", "z");
...
test("two independently created stores never share state - a factory, not a singleton", () => {
  ...
  storeA.recordSaveFailure("mailingStatus", "MAIL-1::2", "x");
